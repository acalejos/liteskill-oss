...
Role-based access control (RBAC) controls **system-wide actions** (e.g., creating conversations, managing MCP servers, admin functions). This is orthogonal to the entity ACL system above, which controls per-resource access.

### roles

Named permission bundles that can be assigned to users and groups.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `id` | `binary_id` | PK, auto-generated | Role identifier |
| `name` | `string` | UNIQUE, NOT NULL | Role name |
| `description` | `string` | | Human-readable description |
| `system` | `boolean` | DEFAULT `false`, NOT NULL | System-managed role (cannot be deleted or renamed) |
| `permissions` | `{:array, :string}` | DEFAULT `[]`, NOT NULL | List of permission strings |
| `inserted_at` | `utc_datetime` | AUTO | |
| `updated_at` | `utc_datetime` | AUTO | |

**System roles** (seeded on boot via `Rbac.ensure_system_roles/0`):
- `Instance Admin` -- Full system access (wildcard `"*"` permission)
- `Default` -- Baseline permissions for all users (create conversations, agents, teams, etc.)
...
Join table assigning roles directly to users.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `id` | `binary_id` | PK, auto-generated | Assignment identifier |
| `user_id` | `binary_id` | FK -> `users.id` (on_delete: delete_all), NOT NULL | User |
| `role_id` | `binary_id` | FK -> `roles.id` (on_delete: delete_all), NOT NULL | Role |
| `inserted_at` | `utc_datetime` | AUTO | |
| `updated_at` | `utc_datetime` | AUTO | |
...
Join table assigning roles to groups. All members of the group inherit the role's permissions.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `id` | `binary_id` | PK, auto-generated | Assignment identifier |
| `group_id` | `binary_id` | FK -> `groups.id` (on_delete: delete_all), NOT NULL | Group |
| `role_id` | `binary_id` | FK -> `roles.id` (on_delete: delete_all), NOT NULL | Role |
| `inserted_at` | `utc_datetime` | AUTO | |
| `updated_at` | `utc_datetime` | AUTO | |

**Indexes:**
- Unique index on `(group_id, role_id)`
- Index on `role_id`
...
| `user_id` | `binary_id` | FK -> `users.id`, NOT NULL | Owner user |
| `run_id` | `binary_id` | FK -> `runs.id` | Associated run (if generated by an agent run) |
| `inserted_at` | `utc_datetime` | AUTO | |
| `updated_at` | `utc_datetime` | AUTO | |
...
The database schema is managed through **46 Ecto migrations** (as of the current version), covering the full schema evolution from the initial event store to the latest usage tracking features. Migrations are located in `priv/repo/migrations/` and are timestamped for ordering.
...
- `20260216100000` -- Add run_id to reports (links reports to agent runs)
- `20260216200000` -- RBAC tables (roles, user_roles, group_roles)
