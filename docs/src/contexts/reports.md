...
| `run_id` | `:binary_id` | FK to Run (if generated by an agent run) |
...
### `create_report(user_id, title, opts \\ [])`

Creates a report with an owner ACL in a transaction.

```elixir
create_report(binary_id, String.t(), keyword())
:: {:ok, Report.t()} | {:error, term()}
```

Options:
- `:run_id` -- associates the report with an agent run
